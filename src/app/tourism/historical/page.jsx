...
import { ArrowLeft, Search, MapPin, Heart, Landmark, HelpCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import Link from 'next/link';
import { MOCK_TOURISM_PLACES } from '@/data/tourism_mock';

export default function HistoricalTourismScreen() {
  const [searchQuery, setSearchQuery] = useState('');

  const places = MOCK_TOURISM_PLACES.filter(place => place.category === 'historical' && (
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-on-surface leading-tight">ചരിത്ര സ്മാരകങ്ങൾ</h2>
          <p className="text-xs text-on-surface-variant font-label tracking-wide uppercase opacity-75 mt-0.5">Historical Heritage Spots</p>
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {places.length > 0 ? (
            places.map(place => (
              ...
              >
                <div className="px-5 pt-5 pb-3 flex justify-between items-center">
                  <span className="text-[10px] font-black tracking-wide uppercase px-2.5 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-200/50 font-label">
                    ചരിത്രം
                  </span>
            ...
            <div className="bg-surface-container-low border border-outline-variant/10 rounded-2xl p-8 text-center flex flex-col items-center justify-center gap-3">
              <Landmark className="w-10 h-10 text-on-surface-variant/40" />
              <p className="text-sm font-bold text-on-surface">ചരിത്ര സ്മാരകങ്ങളൊന്നും കണ്ടെത്താനായില്ല</p>
            </div>
          )}
        </AnimatePresence>
      </div>
